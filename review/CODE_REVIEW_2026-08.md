...
### 10. ~~Email/user enumeration~~ — FIXED 2026-08-17, PARTIALLY REVERTED 2026-08-24
`internal/server/registration.go`, `internal/server/invites.go`, `internal/server/authentication.go` `GetToken()`

**2026-08-24 follow-up:** `Register`'s invite-only branch now deliberately returns `codes.PermissionDenied` ("invitation required") for a non-invited email again, instead of the uniform generic response this fix introduced — an owner-approved, scoped exception (not a regression of this finding): the invite pool is small, invited users register quickly, and completing registration for an invited email requires mailbox access regardless of whether invite status is known, so the mobile app can show a real "invitation required" message. The duplicate-email (`AlreadyExists`) uniform-response protection below, and the `InviteUser`/`GetToken` fixes, are untouched and still fully in effect — this carve-out is scoped to invite-only rejection only. See `CLAUDE.md` and `Docs/AuthImprovement/AUTH_IMPROVEMENTS_PHASE_03.md`.
